...
|---|---|---|---|---|
| FB-01 | Inconsistencias entre rutas directas y Gateway | Se removieron context paths internos y se agrego `StripPrefix=1` | `api-gateway/src/main/resources/application.yml`, `application*.properties` | Corregido |
...
| FB-17 | Trazabilidad en Gateway | Se agrego filtro `X-Request-Id`, preservando el valor entrante o generando UUID, con pruebas automatizadas | `api-gateway/src/main/java/com/ironflow/gateway/config/RequestIdFilter.java`, `SecurityConfigTest` | Cerrado local |
...
cd ../class-service/class-service && ./mvnw test
cd ../../booking-service/booking-service && ./mvnw test
cd ../../trainer-service/trainer-service && ./mvnw test
cd ../../access-service/access-service && ./mvnw test
cd ../../equipment-service/equipment-service/equipment-service && ./mvnw test
cd ../../../branch-service/branch-service && ./mvnw test
cd ../../notification-service/notification-service && ./mvnw test
cd ../../api-gateway && ../MemberShip-service/mvnw test
