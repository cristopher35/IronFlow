...
class ManejadorGlobalExcepcionesTest {
    ...
    void buildsExpectedErrorResponses() {
        // Given
        when(request.getRequestURI()).thenReturn("/api/notificaciones/99");

        // When / Then
        assertEquals(404, handler.manejarNoEncontrado(new RecursoNoEncontradoException("no existe"), request).getStatusCode().value());
        assertEquals(400, handler.manejarSolicitudIncorrecta(new IllegalArgumentException("canal invalido"), request).getStatusCode().value());
